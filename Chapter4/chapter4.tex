...
\section[Thermal properties of black and blue phosphorene]{Thermal properties of black and blue phosphorene \footnote{This work is published:\cite{Aierken2015.thermalP}}  \label{thermal_phos}}
...
The expansion of these lattice parameters due to zero-point vibration is around 0.2\% at 0 K, which is smaller than that of other well-known 2D materials like graphene and $h$-BN, but it is comparable with that of MoS$_2$ and MoSe$_2$. 
...
we also show $\Delta F(T)$ which is defined as $\Delta F(T)$=$F_{blue P}(T)$ - $F_{black P}(T)$. Here, $F_{blue P}(T)$ and $F_{black P}(T)$ are the Helmholtz free energy of black P and blue P, respectively. As the zero-point energy continuously decreases with strain from minus to plus, the variation of the zero-point energy results in an asymmetric behaviour in cohesive energy with strain. In addition, the curvature of this total energy gives the in-plane stiffness of the material as a measure of the response to mechanical deformation. It is clear that blue P is a stiffer material as compared to black P. This is because the deformation in black phosphorene involves bond rotation while this is bond stretching in blue phosphorene, the former costs significant less energy as compared to the latter.  For the same reason, the deformation along the zigzag direction of black P is harder than that along the armchair direction at 0 K, and this is consistent with the finite temperature behaviour as we concluded from the previous section in connection with \autoref{fig:tec}(a). 
...
Barry phase is the extra phase factor appears when one change an external parameter of a quantum system adiabatically. For example, changing the direction of an external magnetic field for a spin-polarized system. In this case, if one do it adiabatically, we would expect the total spin will follow the external field. In reality, it is true but with an extra phase factor and it is called the Barry phase. In the following we will see, for our case of polarization, the parameter is the crystal momentum $k$. The polarization $P$ is defined as electrical dipole moment per unit volume. It is the sum of ionic $P_{ion}$ and electronic $P_e$ part, those are defined as follows:
\begin{equation}
P_{ion}=\frac{1}{\Omega}\sum_jZ_jer_j
\end{equation}
and
\begin{equation}
P_e=\frac{-e}{N\Omega}\int d^3rn(r)r,
\end{equation}
where $\Omega$ is the volume of the unit cell, $j$ runs over the ions in the unit cell with charge $Z_je$ at position $r$, $N$ is the numer of unit cells in the system, $n(r)$ is the electron density at position $r$. Here we have two problems: 1) $P_{ion}$ is not uniquely defined and depends on our choose of unit cell and basis. However, this is rather a small problem compared to the next one since we can stick to one choose of unit cell and basis and keep track of the atoms; 2) In solid, the integral in the expression for $P_e$ will not converge and $N\rightarrow \infty$. More sensible expression can be derived using the concept of Wannier functions $W_n(r)$ and Bloch functions $\psi_{nk}(r)=e^{ik\cdot r}u_{nk}(r)$:
\begin{equation}\begin{aligned}
P_e= & \frac{-e}{\Omega}\sum_n^{occ.}\int d^3r|W_n(r)|^2r \\
= &\frac{-e}{\Omega}\sum_n^{occ.}\frac{1}{N}\sum_{k,k'}\bra{\psi_{nk}}\hat{r}\ket{\psi_{nk'}} \\
= &\frac{-e}{\Omega}\sum_n^{occ.}\frac{1}{N}\left( \sum_k\bra{u_{nk}}i\nabla_k\ket{u_{nk}}\right) ,
\end{aligned}\end{equation}
where $n$ and $k$ are the band index and crystal momentum of Bloch functions. The Wannier functions take the form:
\begin{equation}
W_n(r-R)=\frac{1}{\sqrt{N}}\sum_ke^{-ik\cdot R}\psi_{nk}(r).
\end{equation}
The object in the parentheses represents the extra phase factor. It is equivalent to say that the polarization along one direction of electrons can be expressed in terms of Barry phase that is resulted from adiabatically changing $k$ along that direction. This is a general idea of the modern theory of polarization that is formulated by \citet{King-Smith1993}.
...
\section[Magnetic properties of penta-hexa-graphene]{Magnetic properties of penta-hexa-graphene \footnote{This work is published:\cite{Aierken2016.magnetism}} \label{mag_phg}}
...
\section[Lithium battery related properties of MXenes/ \\ graphene heterostructure]{Lithium battery related properties of MXenes/ \protect\\ graphene heterostructure \footnote{This work is submitted:\cite{Aierken2017.battery}} \label{Li_MG}}
...
In addition, we calculated diffusion barriers for Li ions using the climbing-image nudge elastic (cNEB) method as implemented in the VASP transition state tools\cite{c-neb1,c-neb2}. The cNEB method is an efficient method to determine the minimum energy diffusion path between two given positions. First, we fully relaxed both the initial and final structure that are the beginning and the end of Li diffusion path. Then we interpolate five structures in between, that is five images. During the calculation of the energies of these 5 images, cNEB prohibits relaxation of any of the image towards the previous or the next image, while it is allowed to relax its perpendicular forces. In this way, the Li atom distance between any two images is fixed. In the end, we can plot the coordination of Li atom against total energy for all the images, which gives the diffusion energy barrier profile. The maximum height of the profile gives diffusion barrier that Li atom has to overcome in order to diffuse on the host material. The atomic positions and energy of the images were then relaxed until the largest norm of the force orthogonal to the path is smaller than 0.01 eV/{\AA}. 
...
Using the calculated ground state stacking types of all the M$_2$CX$_2$+Gr heterostructures, we investigated the Li intercalation in these structures. To systematically uncover the potential of M$_2$CX$_2$+Gr heterostructures for battery applications, we also considered Li adsorption on mono-M$_2$CX$_2$, within bi-M$_2$CX$_2$ and within bi-Gr. These additional reference calculations were performed with the same system dimensions, or equivalently the same Li atom concentration, as in the corresponding heterostructures.

Previous studies\cite{Guo2015,Samad2016,Lee2012} pointed out that the strongest binding of the Li atoms occurs for bilayer heterostructure. They reported a non-binding character of Li atoms on the surface of few-layer graphene. Therefore, in this study, we limited ourselves to the intercalation between M$_2$CX$_2$ and graphene. In addition, inspired by the previous study\cite{doi:10.1063/1.4939745} on the location of the Li absorption site in M$_2$CX$_2$ multilayer system, we found that the intercalated Li atom is bound close to the M$_2$CX$_2$ layer and far from graphene, and it resides between three O atoms. Thus, each formula unit of M$_2$CX$_2$ in heterostructure can accommodate one Li atom.  Additional Li atoms occupy the same position in other unit cells, as shown in \autoref{sites}. Note that the ground state stacking has reflection symmetry, and therefore we may limit ourselves to investigate Li positions only on one side of the symmetry line [110] for  single Li absorption.  A strong binding between Li and M$_2$CX$_2$+Gr bilayer is necessary to avoid the formation of metallic lithium, which well improves the safety and reversibility of lithium-ion batteries. Therefore, we need to make sure that the reported Li binding energy as large as possible. We calculated all the possible adsorption sites between the two layers, the results are shown in \autoref{Li-en}.  From these results, we can determine where the Li atom binds most strongly. The relative binding energy differences are generally small at different position as compared to their absolute values in \autoref{my-label}. This is also the reason we ignore the preferred adsorption position where subsequent Li atoms would occupy and apply uniform distribution of Li atoms. In the following, the single Li binding energy refers to the ground state configuration.

\begin{table*}[ht]
{\footnotesize
\centering
\caption{Single Li binding energy $E_b^{Li}$ (eV/Li atom) at different inequivalent positions between heterostructures given as the difference from the strongest binding energy marked in red. Numbers are arranged in accordance with Li atom position in the structures shown in the \autoref{sites}. Note: Sc$_2$CO$_2$+Gr+Li is not stable since it involves strong structure distortion.}
\label{Li-en}
\begin{tabularx}{\textwidth}{lXXXXXXXXXX} \hline
Heterostructure  & \multicolumn{5}{c}{Oxides (X=O)} & \multicolumn{5}{c}{Hydroxides (X=OH)} \\ 
\hline
\multirow{3}{*}{Sc$_2$CX$_2$+Gr+Li} &      &      & 15.614  &      &      &   &  & 0.084   &       &       \\
                  &      &  0.033    &   0.013   &      &   &  & 0.059   &    0.046   &       &      \\
                  &  0.014    &  \textcolor{red}{0.000}    &  14.859    &      &      &   \textcolor{red}{0.000}    &    0.014   &    0.064   &       &       \\
\multicolumn{10}{c}{}  \\               
\multirow{4}{*}{Ti$_2$CX$_2$+Gr+Li} &      &      &      &  0.005    &      &       &       & &  0.159     &  \\
                  &      &      &   0.004   &   0.010   &      &       &    & 0.123   &    0.105   &    \\
                  &      &   0.008   &   0.002   &   0.020   &      &    & 0.095   &   0.135    &    0.029   &      \\
                  &   0.028   &   0.025   &  0.001    &   \textcolor{red}{0.000}   &   & \textcolor{red}{0.000}   &   0.027    &   0.115    &   0.032    &   \\
\multicolumn{10}{c}{}  \\          
\multirow{5}{*}{V$_2$CX$_2$+Gr+Li} &      &      &      &      &   0.623   &       &       &       &       &    0.131   \\
                  &      &      &      &   0.361   &   0.198   &       &       &       &    0.148   &    0.181   \\
                  &      &      &   0.327   &   0.358   &   0.331   &       &       &    0.181   &    0.135   &    0.082   \\
                  &      &  0.350    &   0.334   &   0.351   &   0.331   &       &   0.049    &    0.142   &   0.062    &    0.042   \\
                  &   0.370   &   0.352   &   \textcolor{red}{0.000}   &   0.192   &   0.365   &   \textcolor{red}{0.000}    &   0.010    &    0.089   &    0.137   &   0.055 \\  \hline 
\end{tabularx}
}
\end{table*}
